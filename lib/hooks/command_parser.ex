defmodule Overdiscord.Hooks.CommandParser do
  ...
  def parse_commands() do
    %{
      "h" => %{
        ...
        help: %{
          nil: "Shows all commands or if a command path is passed in then shows the help for that command"
        }},
  ...
  def unhandled_callback(%{cmds: [_self | cmds] = all_cmds, base_parser: parser}, :invalid_callback) do
    case Enum.reduce(cmds, {parser, nil}, fn cmd, {parser, cb} ->
        {parser[:sub_parsers][cmd], parser[:callback] || cb}
      end) do
      {nil, _cb} -> "@OvermindDL1 Parsed Commands `#{inspect all_cmds}` don't actually exist in parser!"
      {_parser, cb} -> "@OvermindDL1 Command `#{inspect all_cmds}` has base invalid callback of: `#{inspect cb}`"
    end
  end
  ...
  def handle_help(%{args: [cmd | _] = args}) do
    case parse_commands()[cmd] do
      nil -> "Command `#{cmd}` does not exist"
      parser ->
    ...
    case cmds do
      ...
      [self | [next | _] = rest] ->
        case sub_parsers[next] do
          nil -> "\t`#{next}` is not a valid sub command of `#{self}`"
          parser ->
  ...
  defp get_callback({module, fun, extra_args} = mfa, _fallback) when is_atom(module) and is_atom(fun) and is_list(extra_args) do
    if :erlang.function_exported(module, fun, 1 + length(extra_args)) do
      mfa
    else
      Logger.warn("Callback `#{module}.#{fun}/#{1 + length(extra_args)}` does not exist, extra args: #{inspect extra_args}")
      {__MODULE__, :unhandled_callback, [:invalid_callback]}
    end
  end
  ...
  req >= 0 and opt >= 0, do: {req, opt - req}
  ...
  defp handle_option_parser_cmd(parser, args, cb, auth, cmd, unparsed_args, opts) do
    ...
    |> Enum.into([])
    |> handle_option_parser_cmd(get_args_required_from_parser(parser), [], [], [cmd], parser, args, cb, auth, cmd, unparsed_args, opts)
  end

  defp handle_option_parser_cmd(_parser_args, {req, opt}, parsed_params, parsed_args, parsed_cmds, parser, ["--" | args], cb, auth, _cmd, unparsed_args, _opts) do
    args_length = length(args)
    cond do
      args_length < req -> {:error, "Missing required arguments"}
      args_length > (req + opt) -> {:error, "Too many arguments"}
      :else ->
        {:ok, %{params: parsed_params, args: Enum.reverse(parsed_args, args), cmds: Enum.reverse(parsed_cmds, callback: cb, auth: auth, unparsed_args: unparsed_args, parser: parser)}}
    end
  end
  defp handle_option_parser_cmd(parser_args, {req, opt}, parsed_params, parsed_args, parsed_cmds, parser, args, cb, auth, cmd, unparsed_args, opts) do
    # IO.inspect({req, opt, args, parsed_params, parsed_args, parsed_cmds}, label: :Parsing)
    case OptionParser.next(args, parser_args) do
      {:ok, key, value, args} -> handle_option_parser_cmd(parser_args, {req, opt}, [{key, value} | parsed_params], parsed_args, parsed_cmds, parser, args, cb, auth, cmd, unparsed_args, opts)
      ...
      {:error, []} -> {:ok, %{params: parsed_params, args: Enum.reverse(parsed_args), cmds: Enum.reverse(parsed_cmds), callback: cb, auth: auth, unparsed_args: unparsed_args, parser: parser}}
      {:error, [subcmd | args]} ->
        if req == 0 do
          case parser[:sub_parsers][subcmd] do # sub command
            nil ->
              case parser[:sub_parsers][nil] do # self is a command
                nil ->
                  if opt == 0 do
                    {:error, "Too many arguments"}
                  else
                    handle_option_parser_cmd(parser_args, {req, opt - 1}, parsed_params, [subcmd | parsed_args], parsed_cmds, parser, args, cb, auth, cmd, unparsed_args, opts)
#{:ok, %{params: parsed_params, args: Enum.reverse(parsed_args, rest), cmds: Enum.reverse(parsed_cmds), callback: cb, auth: auth, unparsed_args: unparsed_args, parser: parser}}
                ...
                parser ->
                  ...
                  |> Map.take([:strict, :aliases])
                  |> Enum.into([])
                  |> handle_option_parser_cmd(get_args_required_from_parser(parser), parsed_params, parsed_args, parsed_cmds, parser, args, get_callback(parser[:callback], cb), auth, cmd, unparsed_args, opts)
              end
            parser ->
              parser
              |> Map.take([:strict, :aliases])
              |> Enum.into([])
              |> handle_option_parser_cmd(get_args_required_from_parser(parser), parsed_params, parsed_args, [subcmd | parsed_cmds], parser, args, get_callback(parser[:callback], cb), auth, cmd, unparsed_args, opts)
          end
        else
          handle_option_parser_cmd(parser_args, {req - 1, opt}, parsed_params, [subcmd | parsed_args], parsed_cmds, parser, args, cb, auth, cmd, unparsed_args, opts)
        end
    end
  end
end
