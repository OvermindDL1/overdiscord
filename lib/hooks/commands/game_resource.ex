defmodule Overdiscord.Hooks.Commands.GameResource do
  ...
  def handle_cmd_status(%{auth: auth} = parsed) do
    ...
    case Game.get(auth) do
      nil -> "Player `#{auth.id}` doesn't have an active game, run `start` to start a game"
      game ->
    ...
    case parsed.args do
      ...
      [item] ->
        case Game.get_construction(game, item) do
          nil -> "Not a valid buildable: #{item}"
          construct ->
  ...
  defmodule Game do
    ...
    def add_resource(game, res, amt, partial \\ 0.0) when amt >= 0 and partial >= 0.0 and partial < 1.0 do
      resource = get_resource(game, res)
      amt = resource.amt + amt
      partial = resource.partial + partial
      {amt, partial} = if(partial >= 1.0, do: {amt + 1, partial - 1.0}, else: {amt, partial})
      cond do
        resource.max === 0 -> {:error, res <> " has no storage"}
        resource.amt === resource.max -> {:error, res <> " is too full"}
        amt > resource.max ->
    ...
    def use_resource(game, res, amt, partial \\ 0.0) when amt >= 0 and partial >= 0.0 and partial < 1.0 do
      resource = get_resource(game, res)
      amt = resource.amt - amt
      partial = resource.partial - partial
      {amt, partial} = if(partial < 0.0, do: {amt - 1, partial + 1.0}, else: {amt, partial})
      cond do
        amt < 0 -> {:error, "not enough " <> res}
        true ->
          resource = %{resource | amt: amt, partial: partial}
          {:ok, update_resource(game, res, resource), resource}
      end
             ...
             %__MODULE__{} = game ->
               ...
               |> Enum.reduce_while(game, fn effect, game ->
                 case construct_effect(game, {name, construction}, effect) do
                   {:ok, game} -> {:cont, game}
                   {:ok, game, _} -> {:cont, game}
                   {:ok, game, _, 0} -> {:cont, game}
                   {:ok, _game, _, leftover} -> {:halt, name <> " does not have enough space to put in resources, missing `#{leftover}` space"}
                   {:error, reason} -> {:halt, reason}
                 end
               end)
               |> case do
