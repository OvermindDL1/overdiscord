defmodule Overdiscord.Web.GithubWebhookController do
  ...
  def handle(conn, %{"action" => "created", "repository" => %{"html_url" => html_url}, "issue" => %{"title" => title}, "comment" => %{"url" => comment_url, "body" => comment_body}} = data) do
    msg = "Issue #{html_url} - #{title}\n#{comment_body|>String.split("\n")|>List.first()|>String.slice(0, 240)}"
